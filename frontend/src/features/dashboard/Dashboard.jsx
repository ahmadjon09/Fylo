...
import { FiPackage, FiAlertTriangle, FiXCircle, FiDollarSign, FiTrendingUp, FiUsers, FiShoppingBag, FiActivity } from 'react-icons/fi';
...
export default function Dashboard() {
  ...
  useEffect(()=>{
    ...
    s.on('product:created', invalidate);
    s.on('product:updated', invalidate);
    s.on('sale:created', invalidate);
    s.on('sale:refunded', invalidate);
    return ()=>{ s.off('product:created', invalidate); s.off('product:updated', invalidate); s.off('sale:created', invalidate); s.off('sale:refunded', invalidate); };
  }, [qc]);
            ...
            {(data?.charts?.topProducts||[]).slice(0,6).map((p,i)=>(
              <motion.div key={i} initial={{ opacity:0, x:-8 }} animate={{ opacity:1, x:0 }} transition={{ delay:i*0.04 }} className="group flex items-center justify-between rounded-[12px] border border-border/60 bg-muted/30 hover:bg-accent/60 p-3 transition-colors">
                ...
                <div className="flex items-center gap-3 text-[12px]">
                  ...
                  <span className="font-[650] tabular-nums text-emerald-600 dark:text-emerald-400">${p.totalProfit?.toFixed(0)}</span>
                </div>
              </motion.div>
            ))}
          </div>
            ...
            <div className="space-y-0 divide-y divide-border/60 -mx-5">
              {(data?.recentSales||[]).slice(0,5).map(s=>(
                <div key={s._id} className="flex items-center justify-between px-5 py-3 hover:bg-accent/40 transition-colors">
                  ...
                  <div className="text-right"><div className="text-[13px] font-[650] tabular-nums">+${s.profit?.toFixed(0)}</div><div className="text-[11px] text-muted-foreground">{s.quantity} {t('dashboard.units')}</div></div>
                </div>
              ))}
            </div>
          </Card>
